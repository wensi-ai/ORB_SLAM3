...
class TelemetryImporter:
    ...
    def _remove_seconds(self, accl, gyro, timestamps_ns, skip_seconds, unit=1e-9):
        skip_ns = skip_seconds / unit
    ...
    def read_insta360_telemetries(self, input_videos, input_imus, gravity_constant, skip_seconds=0.0, window_size=1):
        '''
        input_videos : list of paths to mp4 files
        input_imus : list of paths to txt files for raw telemetry data
        gravity_constant : float
        skip_seconds : float
            How many seconds to cut from beginning and end of stream
        '''
        accl, gyro  = [], []
        timestamps_ns, img_timestamps_ns = [], []
        for video_file, imu_file in zip(input_videos, input_imus):
            self._read_insta360_telemetry(video_file, imu_file, gravity_constant, skip_seconds, window_size)
            accl.extend(self.telemetry["accelerometer"])
            gyro.extend(self.telemetry["gyroscope"])
            timestamps_ns.extend(self.telemetry["timestamps_ns"])
            img_timestamps_ns.extend(self.telemetry["img_timestamps_ns"])
        self.telemetry["accelerometer"] = accl
        self.telemetry["gyroscope"] = gyro
        self.telemetry["timestamps_ns"] = timestamps_ns
        self.telemetry["img_timestamps_ns"] = img_timestamps_ns

    def _read_insta360_telemetry(self, input_video, input_imu, gravity_constant, skip_seconds=0.0, window_size=1):
        '''
        input_video : path to mp4 file
        input_imu : path to txt file for raw telemetry data
        gravity_constant : float
        skip_seconds : float
            How many seconds to cut from beginning and end of stream
        '''
        imu_start, image_start, imu_end, image_end = None, None, None, None
        camera_fps = 24.0
        with open(input_imu, 'r') as f:
            lines = f.readlines()
            for i, l in enumerate(lines):
                if l.split(':', 1)[0].strip() == "Video Frame Rate":
                    camera_fps = float(l.split(':', 1)[1].strip())
                if l.split(':', 1)[0].strip() == "Time Code" and not imu_start:
                    imu_start = i
                if l.split(':', 1)[0].strip() == "Exposure Time" and not image_start:
                    image_start = i - 1
                    imu_end = i - 1
                if (l.split(':', 1)[0].strip() == "Image Size" or l.split(':', 1)[0].strip() == "Shutter Speed") and not image_end:
                    image_end = i
        assert (imu_end - imu_start) % 3 == 0, "IMU data is not in correct format"
        assert (image_end - image_start) % 2 == 0, "Image data is not in correct format"

        accl, gyro  = [], []
        timestamps_ns, img_timestamps_ns = [], []


        for i in range(imu_start, imu_end, 3 * window_size):
            timestamps_ns.append(float(lines[i].split(':')[1]) * self.ms_to_sec / self.ns_to_sec)
            accl_data = lines[i+1].split(':')[1].split()
            gyro_data = lines[i+2].split(':')[1].split()
            accl.append([
                -float(accl_data[1]) * gravity_constant, 
                -float(accl_data[0]) * gravity_constant, 
                -float(accl_data[2]) * gravity_constant
            ])
            gyro.append([-float(gyro_data[1]), -float(gyro_data[0]), -float(gyro_data[2])])
        for i in range(image_start, image_end, 2):
            img_timestamps_ns.append(float(lines[i].split(':')[1]) * self.ms_to_sec / self.ns_to_sec)
        ...
        self.telemetry["accelerometer"] = accl
        self.telemetry["gyroscope"] = gyro
        self.telemetry["timestamps_ns"] = [t + 0.15959290178540944 / self.ns_to_sec for t in timestamps_ns]
        self.telemetry["camera_fps"] = camera_fps
        # for some reason the image timecode has some redundant frames at the beginning
        cap = cv2.VideoCapture(input_video)
        num_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
        cap.release()
        cv2.destroyAllWindows()
        offset = len(img_timestamps_ns) - num_frames
        self.telemetry["img_timestamps_ns"] = img_timestamps_ns[offset:]
...
class TelemetryConverter:
    ...
    def convert_insta360_telemetry(self, input_video, input_txt, output_path, gravity_constant=9.81, skip_seconds=0.0, window_size=1):
        if isinstance(input_video, str):
            input_video = [input_video]
        if isinstance(input_txt, str):
            input_txt = [input_txt]
        self.telemetry_importer.read_insta360_telemetries(input_video, input_txt, gravity_constant, skip_seconds, window_size)
        self._dump_final_json(output_path)

    def convert_insta360_telemetry_to_kalibr(self, input_video, input_txt, output_path, gravity_constant=9.81, skip_seconds=0.0, window_size=1):
        if isinstance(input_video, str):
            input_video = [input_video]
        if isinstance(input_txt, str):
            input_txt = [input_txt]
